...
class ModelProcessor:
    ...
    def _predictionmodelselector(self, training_x, training_y, test_x, testing_y, model_features, model_labels,
        ...
        if len(model_features) == 1 and len(model_labels) == 1 and len(obj_labels_dtypes) == 0:
            ...
            acc_arry = []
            # while True:
            cls = MultiOutputRegressor(
                MLPClassifier(hidden_layer_sizes=(hidden_layer_size, hidden_layer_size, hidden_layer_size),
                              activation="relu", random_state=None, max_iter=2000), n_jobs=-1)
            ...
            acc_arry.append(acc)
                # if acc >= 85 or hidden_layer_size >= 512:
                #     break
                # hidden_layer_size *= 2
